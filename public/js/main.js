...
$(document).mouseup(function(e) {
    ...
    if (!$(".selectInput").is(":focus") && !container.is(e.target) && container.has(e.target).length === 0)
    {
        $(".select").hide();
        if(selectInput) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        result.forEach(element => {
          $(".select table").append("<tr id="+data[element].id+"><th>"+data[element][field]+"</th></tr>");
        });
        $(".select table").append('<tr><td class="cadastrarNovo"><i class="fa fa-plus-circle" aria-hidden="true"></i> Cadastrar novo</td></tr>');
      }
    });
  }
});

$('.selectInput').keyup(delay(function (e) {
  $(".select").show();
  $(".select table").empty();
  ...
  $.ajax({
    type: 'POST',
    url: URL+controller+'/get',
    data: { query: $("#"+controller).parent().find(".selectInput").val() },
    dataType: 'json',
    success: function (data) {
      var result = Object.keys(data);
      $(".select table").empty();
      ...
      if(result.length == 0) {
        $(".select table").append("<tr><td>Nenhum resultado encontrado</td></tr>");
      }
      $(".select table").append('<tr><td class="cadastrarNovo"><i class="fa fa-plus-circle" aria-hidden="true"></i> Cadastrar novo</td></tr>');
    }
  });
...
$(document).on("click",".removeOption", function() {
  ...
  controller = $(this).parent().parent().find('input[type=hidden]').attr('name');
  $("#"+controller).val('');
});
